...
const option = {
  key: fs.readFileSync(path.join(__dirname, "/public/pem/mkcert-key.pem")),
  cert: fs.readFileSync(path.join(__dirname, "/public/pem/mkcert.pem")),
}

const httpServer = http.createServer(isHttps ? option : {}, app)
// 创建信令服务器
...
httpServer.listen(port, "0.0.0.0", () => {
  console.log("\n 服务已经启动 => http://%s:%s", httpServer.address().address, httpServer.address().port)
})
