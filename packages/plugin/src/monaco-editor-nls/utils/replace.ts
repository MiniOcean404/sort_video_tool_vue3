import { fileRelativePathRegexp, isMonacoEditorFile, isNls } from "@/monaco-editor-nls/utils/regexp"

/**
 * 替换编辑器里所有使用获取语言的方法
 */
export function getReplaceMethodCode(path: string, code: string, local: string) {
  if (isMonacoEditorFile(path) && !isNls(path)) {
