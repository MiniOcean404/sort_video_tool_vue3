...
export const chooseAnimation = (animationType: string, grid: HTMLDivElement) => {
  ...
  const timeline = gsap.timeline({
    ...
    scrollTrigger: {
      trigger: gridWrap, // 配置 ScrollTrigger 触发器，指定了何时触发动画，包括滚动触发的范围。
      start: "top bottom+=5%",
      end: "bottom top-=5%",
      scrub: true,
    },
  ...
  switch (animationType) {
    case "grid--1":
      ...
      timeline
        ...
        .fromTo(
          gridItems,
          // xPercent 设置 x 轴元素相对于其自身宽度的水平位置百分比
    ...
    case "grid--6":
      ...
      break
    default:
      console.error("未知的动画类型")
      break
  }
}

const getGrid = (selectors: NodeListOf<Element>) => {
  let elements = gsap.utils.toArray(selectors)
  let bounds

  function getSubset(axis: string, dimension: string, alternating: string, merge: string) {
    let a = [],
      subsets = {},
