// 文章：https://juejin.cn/post/7303075105390297099
// Github: https://github.com/swnb/set-webm-duration/blob/main/src/index.ts
...
function concatArrayBuffer(...buffers: ArrayBuffer[]) {
  const newBuffer = new ArrayBuffer(buffers.reduce((acc, cur) => acc + cur.byteLength, 0))
...
export function setWebmDuration(audioBuffer: ArrayBuffer, duration: number) {
  ...
  const infos = decodeInfoElement(segmentSubElements[infoIndex].data as ArrayBuffer) as EBMLElement[]
